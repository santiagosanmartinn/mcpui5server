import os from "node:os";
import path from "node:path";
import { promises as fs } from "node:fs";
import { spawnSync, execFile } from "node:child_process";
import { promisify } from "node:util";
import { smartStageChangesTool } from "../../src/tools/project/smartStageChanges.js";

const execFileAsync = promisify(execFile);
const gitAvailable = spawnSync("git", ["--version"], { windowsHide: true }).status === 0;
const describeIfGit = gitAvailable ? describe : describe.skip;

describeIfGit("smart_stage_changes", () => {
  let tempRoot;

  beforeEach(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mcp-smart-stage-"));
    await initGitRepo(tempRoot);
    await fs.mkdir(path.join(tempRoot, "webapp", "controller"), { recursive: true });
    await fs.mkdir(path.join(tempRoot, "test"), { recursive: true });
    await fs.mkdir(path.join(tempRoot, "docs"), { recursive: true });
    await fs.writeFile(path.join(tempRoot, "webapp", "controller", "Main.controller.js"), "const v = 1;\n", "utf8");
    await fs.writeFile(path.join(tempRoot, "test", "main.test.js"), "expect(true).toBe(true);\n", "utf8");
    await fs.writeFile(path.join(tempRoot, "docs", "readme.md"), "initial\n", "utf8");
    await git(["add", "."], { cwd: tempRoot });
    await git(["commit", "-m", "initial"], { cwd: tempRoot });
  });

  afterEach(async () => {
    await fs.rm(tempRoot, { recursive: true, force: true });
  });

  it("builds logical staging groups without applying git add", async () => {
    await fs.writeFile(path.join(tempRoot, "webapp", "controller", "Main.controller.js"), "const v = 2;\n", "utf8");
    await fs.writeFile(path.join(tempRoot, "docs", "readme.md"), "updated\n", "utf8");
    await fs.writeFile(path.join(tempRoot, "test", "main.test.js"), "expect(1).toBe(1);\n", "utf8");

    const result = await smartStageChangesTool.handler(
      {
        mode: "working_tree",
        includeUntracked: true
      },
      { context: { rootDir: tempRoot } }
    );

    expect(result.stagingPlan.groups.some((group) => group.id === "ui5-runtime")).toBe(true);
    expect(result.stagingPlan.groups.some((group) => group.id === "docs")).toBe(true);
    expect(result.stagingPlan.groups.some((group) => group.id === "tests")).toBe(true);
    expect(result.automationPolicy.appliesGitAdd).toBe(false);
    expect(result.automationPolicy.requiresExplicitUserConsent).toBe(true);
  });
});

async function initGitRepo(rootDir) {
  await git(["init"], { cwd: rootDir });
  await git(["config", "user.email", "mcp@example.com"], { cwd: rootDir });
  await git(["config", "user.name", "MCP Test"], { cwd: rootDir });
}

async function git(args, options = {}) {
  const cwd = options.cwd ?? process.cwd();
  await execFileAsync("git", args, {
    cwd,
    windowsHide: true
  });
}
